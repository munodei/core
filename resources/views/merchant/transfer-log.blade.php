        ...
        @foreach($sendMoney as $k=>$data)
            <div class="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3">
              <div class="profile-widget">
                ...
                <div class="small text-muted"><strong>Received Date</strong> :  {!! date('d  M, Y  h:i A', strtotime($data->created_at)) !!}</div>
                <div class="small text-muted"><strong>Status</strong> :   @if($data->status == 2)
                      <span  class="badge  badge-pill  badge-success "> completed </span>
                  @elseif($data->status == 1)
                      <span class="badge  badge-pill  badge-danger ">Pending </span>
                  @endif</div>
                <div class="small text-muted"><strong>Invoice</strong> : <a href="{{route('transfer.invoice',$data->trx)}}" class="btn btn-info btn-sm"><i class="fa fa-file-text" aria-hidden="true"></i>
                 </a></div>
              </div>
            </div>
        @endforeach
