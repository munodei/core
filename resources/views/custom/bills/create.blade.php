  ...
  <div class="row align-items-center">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="tile">
                <div class="tile-body">
                    <form role="form" method="POST" action="{{route('bills.store')}}" name="editForm" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                          ...
                          <input type="hidden" name="user_id" id="user_id" value="{{ auth()->user()->id }}"/>
                                    ...
                                    <select name="biller_id" class="form-control form-control-lg">
                                        <option @if(old('biller_id')==='' ) selected @endif value="">Select Biller/Merchant</option>
                                          @foreach($billers as $biller)
                                            <option @if($biller->id ===old('biller_id')) selected @endif value="{{ $biller->id }}"> {{ $biller->biller_name }} ({{ $biller->country->name }})</option>
                                          @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select name="currency" class="form-control form-control-lg">
                                        <option @if(old('currency')==='' ) selected @endif value="">Select Currency</option>
                                          @foreach($countries as $country)
                                            <option @if($country->code ===old('currency')) selected @endif value="{{ $country->code }}"> {{ $country->name }} ({{ $country->code }})</option>
                                          @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="form-group col-md-12">
                                <h5>Name on Bill</h5>
                                <div class="input-group">
                                    <input type="text" value="{{ old('bill_owner') }}" class="form-control form-control-lg" placeholder="Name on Bill" name="bill_owner">
                                </div>
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <div class="input-group">
                                    <input type="text" value="{{ old('bill_account_number') }}" class="form-control form-control-lg" placeholder="Bill Account Number" name="bill_account_number">
                                </div>
