...
<!--     <div class="col-sm-6 col-md-3">
    ...
    <div class="col-sm-6 col-md-3">
      <a href="{{ route('service-request.index') }}"><button class="btn btn-success btn-block"> Request Quotes </button></a>
    </div>
...
<br>
<h2 class="title">Service Provider Main Categories</h2></center>
<br>
    <div class="row">
      ...
      <div class="col-md-4">
          <a href="{{ route('services', ['name' => $category->slug]) }}">
            <div class="card mb-4 shadow-sm">
              <img src="{{url('/') }}/images/upload-nct/main_category_img/{{ $category->img}}" alt="{{ $category->fieldvalue}}" class="bd-placeholder-img card-img-top" width="100%" height="225">
            <div class="card-body">
              <div class="content">
                <center>
  ...
  <div class="container">
    <div class="row justify-content-center mb-5">
      <div class="col-md-7 text-center">
        ...
        <p>Search for any service you need above, or explore some of our more popular categories below</p>
      </div>
    </div>
      ...
      <div class="row">
          ...
          <div class="col-md-4">
            ...
            <div class="card mb-4 shadow-sm">
              <img src="{{url('/') }}/images/upload-nct/category_img/{{ $category->image}}" alt="{{ $category->fieldvalue}}" alt="testimonial image" class="bd-placeholder-img card-img-top" width="100%" height="225">
              <div class="card-body">
                <div class="content">
                    <center><h4 class="title">{{ $category->fieldvalue}}</h4></center>
                </div>
...
<br>
<center><h2 class="title">Featured Service Providers</h2></center>
<div class="row staff-grid-row">
  ...
  @if(count($franchise) >0)
      @foreach($franchise as $k=>$data)
                  ...
                  <a href="{{ route('service_provider.info',['slug'=>$data->post_name]) }}">
                    <div class="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3">
                      <div class="profile-widget">
                        <div class="profile-img">
                                    ...
                                    <a href="{{ route('service_provider.info',['slug'=>$data->post_name]) }}" class="avatar"><center><img src="{{ url('/') }}/assets/{{ $data->meta_value ?? $data->avatar ?? 'images/user/user-default.png' }}" alt="{{ $data->service_provider_user->meta_value }}" width="80px" height="80px"/></center></a>

                        </div>
                        <a href="{{ route('service_provider.info',['slug'=>$data->post_name]) }}">
                        <h4 class="user-name m-t-10 mb-0 text-ellipsis">{{ $data->service_provider_user->display_name }}</h4>
                        <p>{{ $data->post_title }}<p>
                          </a>
                        <br>
                        <br>
      ...
      <div class="col-md-12 col-sm-12 col-12 col-lg-12 col-xl-12">
          <center> You don't have any Service Providers!!</center>
      </div>
...
$url =isset($mainCategories[0]->pagename)?$mainCategories[0]->pagename:'';
 if(url()->current()==url('/essential-service-providers') || url()->current()==url('services/'.$url)){ ?>
jQuery(document).ready(function() {
   var availableTags = <?php echo $sqlSubcat;?>;
   var arr2 = <?php echo $users; ?>;
  autocomplete(document.getElementById("searchtxt"), availableTags,arr2);
});
<?php } ?>
...
inp.addEventListener("input", function(e) {
  ...
  a.setAttribute("style", "background-color: white; text-align: left;height:800px; overflow:scroll;  position: absolute;z-index: 10;");
  /*append the DIV element as a child of the autocomplete container:*/
  this.parentNode.appendChild(a);

	ad = document.createElement("DIV");
	ad.setAttribute("class", "list-group-item-action");
	ad.innerHTML = "<i class='fa fa-user'></i><strong> Users </strong><br>";
	a.appendChild(ad);
  /*for each item in the array...*/
  for (i = 0; i < arr2.length; i++) {
    /*check if the item starts with the same letters as the text field value:*/
    if (arr2[i].value.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
      /*create a DIV element for each matching element:*/
      b = document.createElement("DIV");
      b.setAttribute("class", "list-group-item-action");
      //b.outerHTML  = "<a href='{{ url('/') }}/service/providers/"+ arr[i].slug +"'>";
      /*make the matching letters bold:*/
      b.innerHTML =  "<a href='{{ url('/') }}/service-provider/"+ arr2[i].slug +"'><strong>" + arr2[i].value.substr(0, val.length) + "</strong>";
      b.innerHTML += "<a href='{{ url('/') }}/service-provider/"+ arr2[i].slug +"'>" + arr2[i].value.substr(val.length);
      
      /*insert a input field that will hold the current array item's value:*/
      b.innerHTML += "<input type='hidden' value='" + arr2[i].value + "'>";
      b.innerHTML += "<input type='hidden' value='" + arr2[i].desc + "'>";
      b.innerHTML += "</a>";
	...
	ac.setAttribute("class", "list-group-item-action");
	ac.innerHTML = "<i class='fa fa-paint-brush'></i><strong> Services </strong><br>";
	a.appendChild(ac);
    ...
    /*check if the item starts with the same letters as the text field value:*/
	   if (arr[i].value.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
	      ...
	      /*make the matching letters bold:*/
	      b.innerHTML =  "<a href='{{ url('/') }}/service/providers/"+ arr[i].slug +"'><strong>" + arr[i].value.substr(0, val.length) + "</strong>";
	      b.innerHTML += "<a href='{{ url('/') }}/service/providers/"+ arr[i].slug +"'>" + arr[i].value.substr(val.length);
