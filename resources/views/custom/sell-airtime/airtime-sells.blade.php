...
<div class="page-header">
  <div class="row align-items-center">
    ...
    <div class="col-auto float-right ml-auto">
      <a href="{{ route('sell-airtime.index') }}" class="btn add-btn"><i class="fa fa-plus"></i> Add {{$page_title}}</a>
      ...
      @foreach($sells as $k=>$data)
          <div class="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3">
            <div class="profile-widget">
              ...
              <div class="small text-muted"><strong> Type :</strong> @if($data->airtime_type ==1) Token @else Transfer @endif</div><br>
              <div class="small text-muted"><strong> Token :</strong>  @if($data->airtime_type ==1) {{ $data->token }} @else  Transfer @endif</div>
              <div class="small text-muted"><strong> Status :</strong> {{ $data->status }}</div>
              <div class="small text-muted"><strong> Network :</strong> {{ $data->network }}</div>
              <div class="small text-muted"><strong> Contact Number :</strong> {{ $data->phone }}</div>
              <div class="small text-muted"><strong> Amount Deposited :</strong> {{ $data->amount_to_be_deposited }}</div>
            </div>
          </div>
      @endforeach
      @else
